...
int main(int argc, char **argv) {
  ...
  namePublisher.publish(publishedName);

  // Spin until other robots publish their names.

  while (curRobots != maxRobots);

  // All names are known - decide upon an id.  Do so by looking at the robot's name's position in the array of robot names, which itself is ordered by the time they were published.
